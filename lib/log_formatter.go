...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	lprefix := ""
	if f.Prefix != "" {
		lprefix = colorScheme.PrefixColor("<" + f.Prefix + "> ")
	}
	...
	if len(prefixValue) > 0 {
		prefix = prefix + colorScheme.PrefixColor(" "+prefixValue+":")
		message = trimmedMsg
	} else {
		switch entry.Logger.Level {
		case logrus.TraceLevel:
			prefix = prefix + colorScheme.PrefixColor(" "+getCallStack(true, f.StackLayer)+":")
		case logrus.DebugLevel:
			prefix = prefix + colorScheme.PrefixColor(" "+getCallStack(false, f.StackLayer)+":")
		}
	}
	...
	if f.DisableTimestamp {
		fmt.Fprintf(b, "%s%s%s "+messageFormat, lprefix, level, prefix, message)
	} else {
		...
		fmt.Fprintf(b, "%s %s%s%s "+messageFormat, colorScheme.TimestampColor(timestamp), lprefix, level, prefix, message)
	}
	for _, k := range keys {
		if k != "prefix" {
			v := entry.Data[k]
...
func getCallStack(debug bool, skip int) string {
	up, _, _, _ := runtime.Caller(skip)
	fm, _ := runtime.CallersFrames([]uintptr{up}).Next()
	if debug {
		return fmt.Sprintf("%s:%d", fm.File, fm.Line)
	}
	return fm.Func.Name()
}
